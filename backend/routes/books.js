...
/* Get all books in the db - FIX: Added console.error for 504 issue */
router.get("/allbooks", async (req, res) => {
    try {
        const books = await Book.find({}).populate("transactions").sort({ _id: -1 })
        res.status(200).json(books)
        ...
        console.error("Error fetching all books (504 timeout/error):", err); // <-- CRITICAL FIX
        return res.status(504).json(err);
    }
})

/* Get Book by book Id - FIX: Referenced 'err' in catch block */
router.get("/getbook/:id", async (req, res) => {
    try {
        const book = await Book.findById(req.params.id).populate("transactions")
        res.status(200).json(book)
    }
    catch (err) { // <-- Passed 'err' to catch block
